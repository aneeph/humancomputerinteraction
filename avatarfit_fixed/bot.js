...
async function startBot(exercise) {
  ...
  // Reset skeleton smoothing so the avatar snaps to position on mode start
  if (typeof resetSkeletonSmoothing === 'function') resetSkeletonSmoothing();

  const totalFrames = data.frames.length;
  botInterval = setInterval(() => {
    if (!botCanvas || !onBotFrame) return;

    // Reset smoothing on loop wrap so the avatar doesn't fly on restart
    if (botFrameIdx > 0 && botFrameIdx % totalFrames === 0) {
      if (typeof resetSkeletonSmoothing === 'function') resetSkeletonSmoothing();
    }

    const frame = data.frames[botFrameIdx % totalFrames];
    botFrameIdx++;
