...
// ── Cycle to next exercise (left-hand gesture) ───────
...
function checkStopGesture(joints) {
  ...
  const rightRaised = rw && rs && rw.y < rs.y + 20 && rw.x > cx + sw * 0.7;
  const leftRaised  = lw && ls && lw.y < ls.y + 20 && lw.x < cx - sw * 0.7;
...
function startLiveWebcam() {
  ...
  camera.start()
    .then(() => {
      ...
      // Permission granted — now wait for user to step into frame
...
function startWorkoutMode() {
  ...
  updateWorkoutUI();
  sessionEnded = false;
  sessionPhase = 'waiting_frame';  // paused until user taps Let's Go
  showExIntro();
}
