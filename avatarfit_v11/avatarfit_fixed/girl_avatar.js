...
function _zHead(ctx, ls, rs, lh, rh, isCorrect, celebrating, sw) {
  ...
  // ── Ponytail (drawn behind head) ──────────────────────────────────
  if (!celebrating) {
    ctx.save();
    ctx.translate(hc.x, hc.y);
    ctx.rotate(shA - Math.PI / 2);
    ctx.strokeStyle = '#4a3728';
    ctx.lineWidth   = headR * 0.26;
    ctx.lineCap     = 'round';
    ctx.beginPath();
    ctx.moveTo(-headR * 0.42, -headR * 0.05);
    ctx.bezierCurveTo(
      -headR * 0.95,  headR * 0.4,
      -headR * 0.72,  headR * 1.15,
      -headR * 0.28,  headR * 1.5
    );
    ctx.stroke();
    ctx.restore();
  }
  ...
  if (!celebrating) {
    ctx.beginPath();
    ctx.arc(-headR * 0.52, -headR * 0.68, headR * 0.18, 0, Math.PI * 2);
